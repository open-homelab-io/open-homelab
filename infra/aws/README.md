...
- `open-homelab-terraform-state`: S3 bucket used for Terraform/CDKTN remote state. Enable with `AWS_TERRAFORM_STATE_ENABLED=1`.
...
bun install
bun run typecheck
AWS_TERRAFORM_STATE_ENABLED=1 AWS_DNS_AUTOMATION_ENABLED=1 bun run synth
```
...
```text
open-homelab-terraform-state-${AWS_ACCOUNT_ID}-${AWS_REGION}
```
...
After deployment, set `TF_STATE_BUCKET`, `TF_STATE_KEY`, and `AWS_REGION` in the root `.env`, then migrate the Proxmox Terraform state:
